...
int undergraduate :: check_name(string n)
{
	if(name==n)
	{
...
int undergraduate :: check_delay(int y,int m,int d)
{
	...
	if(delay<=0)
	{				
		cout<<"6	restricted member until "<<delay_year<<"/"<<delay_month<<"/"<<delay_day<<endl;
		return -1;
	...
	else
	{
		...
		if(borrow.at(0)==resource_name && borrow_type.at(0)==resource_type)
		{
			...
			else
			{
				...
				cout<<"7	Delayed return. You'll be restricted until "<<delay_year<<"/"<<delay_month<<"/"<<delay_day<<endl;
			}
			borrow_number = 0;
			cout<<"success"<<endl;
